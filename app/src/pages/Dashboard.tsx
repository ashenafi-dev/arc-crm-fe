...
export function Dashboard() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-xs font-medium uppercase tracking-widest text-[var(--gold-500)]">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard label="Active Labor Requests" value={activeLabor} accent="blue" />
        <StatCard label="Delayed Requests" value={delayed} accent="red" />
      </div>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <GlassCard className="p-5 lg:col-span-2">
          <h3 className="mb-4 text-sm font-semibold text-white">Spend by Project</h3>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={spendByProject}>
              ...
              <Tooltip
                contentStyle={{ background: '#14293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, fontSize: 12 }}
        ...
        <GlassCard className="p-5">
          <h3 className="mb-4 text-sm font-semibold text-white">Requests by Status</h3>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              ...
              <Tooltip contentStyle={{ background: '#14293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, fontSize: 12 }} />
            </PieChart>
          </ResponsiveContainer>
        </GlassCard>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <GlassCard className="p-5 lg:col-span-2">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link to="/requests" className="text-xs font-medium text-[var(--gold-500)] hover:underline">View all</Link>
          </div>
          <div className="space-y-2">
            ...
            {actionQueue.map((r) => (
              <Link
                key={r.id}
                to={`/requests/${r.id}`}
                className="focus-ring flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.03] px-3.5 py-3 transition-colors hover:bg-white/[0.06]"
              >
                ...
                <div className="flex shrink-0 items-center gap-3">
                  <span className="text-sm font-semibold text-slate-300">${Number(r.estimated_amount).toLocaleString()}</span>
          ...
          <div className="space-y-3">
            {activity.map((a) => (
              <div key={a.id} className="flex gap-2.5 text-xs">
                <div className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--gold-500)]" />
                <div>
      ...
      {labor.length > 0 && (
        <GlassCard className="p-5">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-white">Active Labor Requests</h3>
            <Link to="/labor" className="text-xs font-medium text-[var(--gold-500)] hover:underline">View all</Link>
          </div>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
            {labor.slice(0, 3).map((l) => (
              <div key={l.id} className="rounded-xl border border-white/5 bg-white/[0.03] p-3.5">
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs text-slate-500">{l.request_number}</span>
                  <LaborStatusBadge status={l.status} />
                </div>
                <p className="text-sm font-medium text-white">{l.labor_type}</p>
