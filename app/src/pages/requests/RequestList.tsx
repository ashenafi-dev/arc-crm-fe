    ...
    <div className="space-y-5">
      ...
      <GlassCard className="flex flex-wrap items-center gap-3 p-3">
        <input
          ...
          className="focus-ring min-w-[220px] flex-1 rounded-lg border border-white/10 bg-white/5 px-3.5 py-2 text-sm text-white placeholder:text-slate-500"
        />
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as RequestStatus | 'all')}
          className="focus-ring rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200"
        >
          ...
          <tbody>
            ...
            {filtered.map((r) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-slate-300">{r.project?.name}</td>
                <td className="hidden px-4 py-3 text-slate-300 md:table-cell">{r.requester?.full_name}</td>
                <td className="px-4 py-3 font-medium text-slate-200">${Number(r.estimated_amount).toLocaleString()}</td>
