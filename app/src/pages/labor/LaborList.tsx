      ...
      {showForm && (
        <GlassCard strong className="p-6">
          <form onSubmit={handleCreate} className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <select value={form.project_id} onChange={(e) => setForm((f) => ({ ...f, project_id: e.target.value }))} className="focus-ring rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-slate-200">
              <option value="">Select project</option>
              {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <input placeholder="Labor type (e.g. Electricians)" value={form.labor_type} onChange={(e) => setForm((f) => ({ ...f, labor_type: e.target.value }))} className="focus-ring rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white" />
            <input type="number" placeholder="Workers required" value={form.workers_required} onChange={(e) => setForm((f) => ({ ...f, workers_required: e.target.value }))} className="focus-ring rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white" />
            <input placeholder="Location" value={form.location} onChange={(e) => setForm((f) => ({ ...f, location: e.target.value }))} className="focus-ring rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white" />
            <input type="datetime-local" value={form.required_at} onChange={(e) => setForm((f) => ({ ...f, required_at: e.target.value }))} className="focus-ring rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white" />
            <input placeholder="Description" value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} className="focus-ring rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white" />
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        {labor.map((l) => (
          <GlassCard key={l.id} className="p-4">
            ...
            <p className="font-medium text-white">{l.labor_type}</p>
            <p className="mt-1 text-xs text-slate-400">{l.project?.name}</p>
            <p className="mt-2 text-sm text-slate-300">{l.workers_required} workers · {l.location}</p>
            <p className="mt-1 text-xs text-slate-500">{new Date(l.required_at).toLocaleString()}</p>
            {canAdvance && NEXT[l.status] && (
              <Button variant="outline" className="mt-3 w-full justify-center" onClick={() => advance(l)}>
