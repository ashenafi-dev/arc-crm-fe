...
-- The on_auth_user_created trigger creates their profiles automatically.
...
select 'PR-2026-0001', 'Structural steel beams', 'Steel beams for phase 2 framing', p.id, 'Architecture', u.id, 185000, 'Required for phase 2 structural work', 'awaiting_finance'
from p, u where p.code = 'PRJ-001' and u.email = 'employee@demo.ethix.io'
union all
select 'PR-2026-0002', 'Site safety equipment', 'Helmets, harnesses, signage', p.id, 'Architecture', u.id, 42000, 'Mandatory site safety compliance', 'awaiting_finance'
from p, u where p.code = 'PRJ-002' and u.email = 'employee@demo.ethix.io'
union all
select 'PR-2026-0003', 'Interior lighting fixtures', 'LED fixtures for common areas', p.id, 'Architecture', u.id, 76500, 'Interior fit-out phase', 'awaiting_gm'
from p, u where p.code = 'PRJ-003' and u.email = 'employee@demo.ethix.io'
union all
select 'PR-2026-0004', 'Plumbing materials batch 3', 'Pipes and fittings restock', p.id, 'Architecture', u.id, 31200, 'Ongoing plumbing installation', 'awaiting_owner'
from p, u where p.code = 'PRJ-001' and u.email = 'employee@demo.ethix.io'
union all
select 'PR-2026-0005', 'Office furniture - reception', 'Reception desk and seating', p.id, 'Operations', u.id, 58900, 'New reception area furnishing', 'approved'
from p, u where p.code = 'PRJ-003' and u.email = 'employee@demo.ethix.io'
union all
select 'PR-2026-0006', 'Landscaping materials', 'Soil, plants, irrigation parts', p.id, 'Architecture', u.id, 24800, 'Exterior landscaping phase', 'completed'
from p, u where p.code = 'PRJ-002' and u.email = 'employee@demo.ethix.io'
union all
select 'PR-2026-0007', 'Generator maintenance parts', 'Replacement parts for backup generator', p.id, 'Operations', u.id, 15600, 'Scheduled maintenance', 'completed'
from p, u where p.code = 'PRJ-001' and u.email = 'employee@demo.ethix.io'
union all
select 'PR-2026-0008', 'Decorative stone cladding', 'Facade cladding material', p.id, 'Architecture', u.id, 112000, 'Exceeded approved budget threshold', 'rejected'
from p, u where p.code = 'PRJ-003' and u.email = 'employee@demo.ethix.io'
union all
select 'PR-2026-0009', 'HVAC ducting materials', 'Ducting for third floor', p.id, 'Architecture', u.id, 67300, 'HVAC rollout phase 3', 'completed'
from p, u where p.code = 'PRJ-001' and u.email = 'employee@demo.ethix.io';

-- Approval rows matching each request's stage
insert into approvals (purchase_request_id, stage, decision)
select id, 'finance', 'pending' from purchase_requests where request_number in ('PR-2026-0001','PR-2026-0002');
insert into approvals (purchase_request_id, stage, decision)
select id, 'gm', 'pending' from purchase_requests where request_number = 'PR-2026-0003';
insert into approvals (purchase_request_id, stage, decision)
select id, 'owner', 'pending' from purchase_requests where request_number = 'PR-2026-0004';
...
select 'LR-2026-0001', p.id, u.id, 'Electricians', 4, 'Riverside Site - Block A', now() + interval '3 days', 'Wiring installation for floors 2-3', 'assigned'
from p, u where p.code = 'PRJ-001' and u.email = 'employee@demo.ethix.io'
union all
select 'LR-2026-0002', p.id, u.id, 'General laborers', 8, 'Bole Site', now() + interval '5 days', 'Site cleanup and material handling', 'reviewed'
from p, u where p.code = 'PRJ-002' and u.email = 'employee@demo.ethix.io'
union all
select 'LR-2026-0003', p.id, u.id, 'Plumbers', 3, 'City Center Site', now() + interval '1 day', 'Bathroom fixture installation', 'in_progress'
from p, u where p.code = 'PRJ-003' and u.email = 'employee@demo.ethix.io';
